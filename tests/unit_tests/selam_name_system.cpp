...
TEST(selam_name_system, name_tests)
{
  struct name_test
  {
    std::string name;
  ...
  for (uint16_t type16 = 0; type16 < static_cast<uint16_t>(ons::mapping_type::_count); type16++)
  {
    ...
    size_t names_count     = ons::is_lokinet_type(type) ? selam::char_count(lokinet_names) : selam::char_count(session_wallet_names);
...
TEST(selam_name_system, value_encrypt_and_decrypt)
{
...
TEST(selam_name_system, value_encrypt_and_decrypt_heavy)
{
