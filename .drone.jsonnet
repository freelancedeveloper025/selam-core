...
local docker_base = 'registry.selam.rocks/lokinet-ci-';
...
local debian_pipeline(name,
                      ...
                      werror=false,  // FIXME
                      build_tests=true,
                      test_selamd=true,  // Simple selamd offline startup test
  ...
  steps: [
    submodules,
    {
      ...
      commands: [
        ...
        + (if test_selamd then 'gdb ' else '') + std.join(' ', deps),
        'mkdir build',
        'cd build',
        ...
        if test_selamd then [
          '(sleep 3; echo "status\ndiff\nexit") | TERM=xterm ../utils/build_scripts/drone-gdb.sh ./bin/selamd --offline --data-dir=startuptest',
        ] else []
      ) + (
        if run_tests then [
          'mkdir -v -p $$HOME/.selam',
...
local gui_wallet_step(image, wine=false) = {
  ...
  commands: (if wine then ['dpkg --add-architecture i386'] else []) + [
    ...
    'cp -v build/bin/selamd' + (if wine then '.exe' else '') + ' loki-electron-gui-wallet/bin',
    'cp -v build/bin/selam-wallet-rpc' + (if wine then '.exe' else '') + ' loki-electron-gui-wallet/bin',
...
local gui_wallet_step_darwin = {
  ...
  commands: [
    'git clone https://github.com/loki-project/loki-electron-gui-wallet.git',
    'cp -v build/bin/{selamd,selam-wallet-rpc} loki-electron-gui-wallet/bin',
    'cd loki-electron-gui-wallet',
    'sed -i -e \'s/^\\\\( *"version": ".*\\\\)",/\\\\1-${DRONE_COMMIT_SHA:0:8}",/\' package.json',
    'npm install',
    'npm run build',
