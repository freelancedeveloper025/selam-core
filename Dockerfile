...
RUN set -ex && \
    ldd /src/build/release/bin/selamd
...
# Create selam user
RUN adduser --system --group --disabled-password selam && \
	mkdir -p /wallet /home/selam/.selam && \
	chown -R selam:selam /home/selam/.selam && \
	chown -R selam:selam /wallet
...
ENTRYPOINT ["selamd", "--p2p-bind-ip=0.0.0.0", "--p2p-bind-port=18171", "--rpc-bind-ip=0.0.0.0", "--rpc-bind-port=18172", "--non-interactive", "--confirm-external-bind"]
