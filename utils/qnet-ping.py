...
if len(sys.argv) < 3:
    ...
    if m:
        selamrpc = "http://{}:{}/json_rpc".format(m.group(1), m.group(2))
        r = requests.post(selamrpc, json={"jsonrpc":"2.0", "id":0, "method":"get_service_node_privkey"}).json()
        if "result" in r and "service_node_x25519_privkey" in r["result"]:
            ...
            print("Loaded x25519 keys (pub: {}) from selamd @ {}".format(hexstr(x_key.public_key), sys.argv[1]))
        else:
            x_key = PrivateKey.generate()
            print("Generated x25519 key {} (selamd @ {} did not return SN privkeys)".format(hexstr(x_key.public_key), sys.argv[1]))
    else:
        print("Error: {} does not look like a valid selamd RPC host:port value".format(sys.argv[1]), sys.stderr)
        badargs = True
...
if badargs:
    print("\nUsage: {0} localhost:18172 PUBKEY -- pings PUBKEY's quorumnet, looking up the address via the given RPC address.\n"
...
if direct:
    ...
    r = requests.post(selamrpc, json={"jsonrpc":"2.0", "id":0, "method":"get_service_nodes", "params": {
