...
class Daemon(RPCDaemon):
    ...
    def __init__(self, *,
        ...
        if name is None:
            name = 'selamd@{}'.format(self.rpc_port)
        super().__init__(name)
        ...
        self.args = [selamd] + list(self.__class__.base_args)
        self.args += (
                '--data-dir={}/selam-{}-{}'.format(datadir or '.', self.listen_ip, self.rpc_port),
                '--log-level={}'.format(log_level),
                '--log-file=selam.log'.format(self.listen_ip, self.p2p_port),
    ...
    def ping(self, *, storage=True, lokinet=True):
        """Sends fake storage server and lokinet pings to the running selamd"""
        if storage:
