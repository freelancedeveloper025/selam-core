...
#define SELAM_DEFAULT_LOG_CATEGORY "serialization"
    ...
    if(escape_mode/*prev_ch == '\\'*/)
    {
      ...
      case 'u':  //Unicode code point
        ...
        else
        {
          ...
          {
            const auto c = *++it;
            CHECK_AND_ASSERT_THROW_MES(selamc::is_hex_digit(c), "Bad Unicode encoding: " + std::to_string(c));
            dst = dst << 4 | selamc::from_hex_digit(c);
          }
          // encode as UTF-8
          if (dst <= 0x7f)
          {
