...
// testnet/devnet/fakenet have always had a fixed 100 SELAM staking requirement:
...
constexpr uint64_t burn_needed(cryptonote::hf hf_version, mapping_type type)
{
  ...
  const uint64_t basic_fee = (
      hf_version >= cryptonote::hf::hf18       ? 7*selam::COIN :
      hf_version >= cryptonote::hf::hf16_pulse ? 15*selam::COIN :
      20*selam::COIN
  );
  switch (type)
  {
