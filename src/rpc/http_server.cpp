...
namespace cryptonote::rpc {
  ...
  // Invokes the actual RPC request; this is called (via selammq) from some random LMQ worker thread,
  ...
  static std::unordered_set<selammq::SelamMQ*> timer_started;
  ...
  void http_server::start()
  {
    ...
    m_server_header = "selamd/"s + (m_restricted ? std::to_string(SELAM_VERSION[0]) : std::string{SELAM_VERSION_FULL})
