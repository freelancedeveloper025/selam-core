...
namespace cryptonote::rpc {
  ...
  std::optional<std::string> check_authorization(std::string_view auth_header, std::string_view realm, Callback check_login) {
    ...
    if (parts.size() < 2 || parts[0] != "Basic"sv || !selamc::is_base64(parts[1]))
      return fail;
    auto login = selamc::from_base64(parts[1]);
  ...
  {
    if (auto www_auth = check_authorization(req.getHeader("authorization"), "selamd rpc",
  ...
  std::string http_server_base::get_remote_address(HttpResponse& res) {
    ...
    else if (addr.size() == 16)
    {
      ...
      for (auto& x : a) selamc::big_to_host_inplace(x);
      ...
      result << "{unknown:" << selamc::to_hex(addr) << "}";
    return result.str();
  }
