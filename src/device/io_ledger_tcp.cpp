...
#define SELAM_DEFAULT_LOG_CATEGORY "device.io"
...
void ledger_tcp::connect() {
  ...
  auto closer = selam::defer([&] { close(fd); });
  ...
  auto addr_free = selam::defer([&] { freeaddrinfo(addr); });
...
int ledger_tcp::exchange(const unsigned char* command, unsigned int cmd_len, unsigned char* response, unsigned int max_resp_len, bool user_input) {
  ...
  uint32_t size = selamc::host_to_big(cmd_len);
  ...
  auto data_size = selamc::big_to_host(size) + 2;
