...
void simple_wallet::list_signers(const std::vector<mms::authorized_signer> &signers)
{
  ...
  message_writer() << boost::format("%2s %-20s %-s") % "" % tr("Auto-Config Token") % tr("Selam Address");
...
void simple_wallet::mms_signer(const std::vector<std::string> &args)
{
  ...
  {
    fail_msg_writer() << tr("mms signer [<number> <label> [<transport_address> [<selam_address>]]]");
    return;
  }
  ...
  if (args.size() == 4)
  {
    ...
    if (!ok)
    {
      fail_msg_writer() << tr("Invalid Selam address");
      return;
    }
    ...
    {
      fail_msg_writer() << tr("Wallet state does not allow changing Selam addresses anymore");
      return;
    }
  }
