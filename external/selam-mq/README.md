# SelamMQ - high-level zeromq-based message passing for network-based projects

This C++17 library contains an abstraction layer around ZeroMQ to provide a high-level interface to
authentication, RPC, and message passing.  It is used extensively within Selam projects (hence the
name) as the underlying communication mechanism of SN-to-SN communication ("quorumnet"), the RPC
interface used by wallets and local daemon commands, communication channels between selamd and
...
requires knowing the server pubkey.  Within Selam, all SN-to-SN traffic is encrypted, and other
...
SelamMQ messages come in two fundamental forms: "commands", consisting of a command named and
...
either direction at any time.  SelamMQ sets up a "listener" and "client" connections, but these only
...
`category.command` - for commands/requests registered by the SelamMQ caller (e.g. selamd).  Here
...
on this below).  For example, for selamd categories are:
...
the request via SelamMQ you provide a callback to be invoked when the reply arrives.  On the wire
this looks like:
...
by SelamMQ (both the invoker and the recipient code only see the `hello world`/`dlrow olleh` message
parts).
...
Optional command/request arguments are always strings on the wire.  The SelamMQ-using developer is
...
Internal commands passed between SelamMQ-managed threads use either plain strings or bt-encoded
dictionaries.  See `selammq/bt_serialize.h` if you want a bt serializer/deserializer.
...
- a string containing a service node pubkey.  In this mode SelamMQ will look for the given SN in
  already-established connections, reusing a connection if one exists.  If no connection already
  exists, a new connection to the given SN is attempted (this requires constructing the SelamMQ
...
The callbacks are passed a SelamMQ::Message object from which the message (plus various connection
...
connection to deliver the message.  In order for this to work the SelamMQ caller must provide a
lookup function to retrieve the remote address given a SN x25519 pubkey.

### Callbacks

Invoked command functions are always invoked with exactly one arguments: a non-const SelamMQ::Message
reference from which the connection info, SelamMQ object, and message data can be obtained.
...
also allows configuration of how public connections should be treated: for example, an selamd running
...
For example, in selamd the categories described above have authentication levels of:
...
In order to handle ServiceNode authentication, SelamMQ uses an Allow callback invoked during
...
SelamMQ operates a pool of worker threads to handle jobs.  The simplest use just allocates new jobs
...
As mentioned above, SelamMQ tries to avoid exceeding the configured general threads value (G)
...
Instead SelamMQ provides a mechanism for this by allowing you to submit a batch of jobs with a
...
void continue_big_task(std::vector<selammq::job_result<double>> results) {
    double sum = 0;
    for (auto& r : results) {
        try {
            sum += r.get();
...
void start_big_task() {
    ...
    selammq::Batch<double /*return type*/> batch;
...
exhausting the worker threads with waiting jobs) then you can implement it yourself; SelamMQ isn't
...
SelamMQ supports scheduling periodic tasks via the `add_timer()` function.  These timers have an
