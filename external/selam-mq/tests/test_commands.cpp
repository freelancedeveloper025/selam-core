...
TEST_CASE("basic commands", "[commands]") {
    ...
    SelamMQ client{get_logger("C» "), LogLevel::trace};
    ...
    wait_for_conn(got);
    {
        ...
        REQUIRE( selamc::to_hex(pubkey) == selamc::to_hex(server.get_pubkey()) );
    }
    ...
    reply_sleep();
    {
        ...
        REQUIRE( hellos == 1 );
        REQUIRE( his == 1 );
        REQUIRE( selamc::to_hex(client_pubkey) == selamc::to_hex(client.get_pubkey()) );
    }
...
TEST_CASE("deferred replies on incoming connections", "[commands][hey google]") {
    ...
    server.add_request_command("hey google", "remember", [&](Message& m) {
            ...
            if (bd)
                m.selammq.send(backdoor, "backdoor.data", m.data[0]);
    });
    ...
    std::vector<std::unique_ptr<SelamMQ>> clients;
    ...
    for (int i = 0; i < 5; i++) {
        clients.push_back(std::make_unique<SelamMQ>(
...
TEST_CASE("data parts", "[commands][send][data_parts]") {
    ...
    client.send(c, "public.hello", selammq::send_option::data_parts(some_data.begin(), some_data.end()));
    reply_sleep();
    {
        auto lock = catch_lock();
        std::lock_guard l{mut};
    ...
    client.send(c, "public.hello",
            "hi",
            selammq::send_option::data_parts(some_data2.begin(), some_data2.end()),
            "another",
            "string"sv,
            selammq::send_option::data_parts(some_data.begin(), some_data.end()),
            opt1, opt2, opt3, opt4
            );
