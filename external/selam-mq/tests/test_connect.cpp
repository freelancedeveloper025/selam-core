...
TEST_CASE("plain-text connections", "[plaintext][connect]") {
    std::string listen = random_localhost();
    SelamMQ server{get_logger("S» "), LogLevel::trace};
...
TEST_CASE("post-start listening", "[connect][listen]") {
    ...
    SelamMQ client{get_logger("C1» "), LogLevel::trace};
    client.start();
...
TEST_CASE("unique connection IDs", "[connect][id]") {
    ...
    SelamMQ client1{get_logger("C1» "), LogLevel::trace};
    SelamMQ client2{get_logger("C2» "), LogLevel::trace};
    client1.start();
    client2.start();
...
TEST_CASE("SN disconnections", "[connect][disconnect]") {
    std::vector<std::unique_ptr<SelamMQ>> omq;
    ...
    for (int i = 0; i < pubkey.size(); i++) {
        omq.push_back(std::make_unique<SelamMQ>(
...
TEST_CASE("SN single worker test", "[connect][worker]") {
    ...
    SelamMQ client{get_logger("B» "), LogLevel::trace};
    client.start();
...
TEST_CASE("inproc connections", "[connect][inproc]") {
    ...
    SelamMQ omq{get_logger("OMQ» "), LogLevel::trace};
