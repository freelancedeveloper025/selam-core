...
bool worker_wait_for(SelamMQ& omq, zmq::socket_t& sock, std::vector<zmq::message_t>& parts, const std::string_view worker_id, const std::string_view expect) {
    while (true) {
...
void SelamMQ::worker_thread(unsigned int index, std::optional<std::string> tagged, std::function<void()> start) {
    ...
    if (tagged) {
        // If we're a tagged worker then we got started up before SelamMQ started, so we need to wait
        // for an all-clear signal from SelamMQ first, then we fire our `start` callback, then we can
    ...
    while (true) {
        ...
        catch (const selamc::bt_deserialize_invalid& e) {
...
SelamMQ::run_info& SelamMQ::get_idle_worker() {
...
void SelamMQ::proxy_worker_message(std::vector<zmq::message_t>& parts) {
    ...
    unsigned int worker_id = selamc::detail::extract_unsigned(worker_id_str);
...
void SelamMQ::proxy_run_worker(run_info& run) {
...
void SelamMQ::proxy_to_worker(int64_t conn_id, zmq::socket_t& sock, std::vector<zmq::message_t>& parts) {
...
void SelamMQ::inject_task(const std::string& category, std::string command, std::string remote, std::function<void()> callback) {
    if (!callback) return;
    auto it = categories.find(category);
    if (it == categories.end())
        throw std::out_of_range{"Invalid category `" + category + "': category does not exist"};
    detail::send_control(get_control_socket(), "INJECT", selamc::bt_serialize(detail::serialize_object(
                injected_task{it->second, std::move(command), std::move(remote), std::move(callback)})));
}

void SelamMQ::proxy_inject_task(injected_task task) {
