...
bool SelamMQ::proxy_check_auth(int64_t conn_id, bool outgoing, const peer_info& peer,
        zmq::message_t& cmd, const cat_call_t& cat_call, std::vector<zmq::message_t>& data) {
    auto command = view(cmd);
    std::string reply;

    if (!cat_call.first) {
        OMQ_LOG(warn, "Invalid command '", command, "' sent by remote [", selamc::to_hex(peer.pubkey), "]/", peer_address(cmd));
        reply = "UNKNOWNCOMMAND";
    } else if (peer.auth_level < cat_call.first->access.auth) {
        OMQ_LOG(warn, "Access denied to ", command, " for peer [", selamc::to_hex(peer.pubkey), "]/", peer_address(cmd),
                ": peer auth level ", peer.auth_level, " < ", cat_call.first->access.auth);
        reply = "FORBIDDEN";
    } else if (cat_call.first->access.local_sn && !local_service_node) {
        OMQ_LOG(warn, "Access denied to ", command, " for peer [", selamc::to_hex(peer.pubkey), "]/", peer_address(cmd),
                ": that command is only available when this SelamMQ is running in service node mode");
        ...
        OMQ_LOG(warn, "Received an invalid request for '", command, "' with no reply tag from remote [",
                selamc::to_hex(peer.pubkey), "]/", peer_address(cmd));
        reply = "NO_REPLY_TAG";
    } else {
        return true;
    }
    ...
    try {
        ...
        OMQ_LOG(debug, "Couldn't send auth failure message ", reply, " to peer [", selamc::to_hex(peer.pubkey), "]/", peer_address(cmd), ": ", err.what());
    }

    return false;
}

void SelamMQ::set_active_sns(pubkey_set pubkeys) {
    if (proxy_thread.joinable()) {
        auto data = selamc::bt_serialize(detail::serialize_object(std::move(pubkeys)));
        detail::send_control(get_control_socket(), "SET_SNS", data);
    } else {
        proxy_set_active_sns(std::move(pubkeys));
    }
}
void SelamMQ::proxy_set_active_sns(std::string_view data) {
    proxy_set_active_sns(detail::deserialize_object<pubkey_set>(selamc::bt_deserialize<uintptr_t>(data)));
}
void SelamMQ::proxy_set_active_sns(pubkey_set pubkeys) {
    pubkey_set added, removed;
    for (auto it = pubkeys.begin(); it != pubkeys.end(); ) {
        auto& pk = *it;
        if (pk.size() != 32) {
            OMQ_LOG(warn, "Invalid private key of length ", pk.size(), " (", selamc::to_hex(pk), ") passed to set_active_sns");
            it = pubkeys.erase(it);
            continue;
        }
...
void SelamMQ::update_active_sns(pubkey_set added, pubkey_set removed) {
    if (proxy_thread.joinable()) {
        std::array<uintptr_t, 2> data;
        data[0] = detail::serialize_object(std::move(added));
        data[1] = detail::serialize_object(std::move(removed));
        detail::send_control(get_control_socket(), "UPDATE_SNS", selamc::bt_serialize(data));
    } else {
        proxy_update_active_sns(std::move(added), std::move(removed));
    }
}
void SelamMQ::proxy_update_active_sns(selamc::bt_list_consumer data) {
...
void SelamMQ::proxy_update_active_sns(pubkey_set added, pubkey_set removed) {
    ...
    for (auto it = removed.begin(); it != removed.end(); ) {
        const auto& pk = *it;
        if (pk.size() != 32) {
            OMQ_LOG(warn, "Invalid private key of length ", pk.size(), " (", selamc::to_hex(pk), ") passed to update_active_sns (removed)");
            it = removed.erase(it);
    ...
    for (auto it = added.begin(); it != added.end(); ) {
        const auto& pk = *it;
        if (pk.size() != 32) {
            OMQ_LOG(warn, "Invalid private key of length ", pk.size(), " (", selamc::to_hex(pk), ") passed to update_active_sns (added)");
            it = added.erase(it);
...
void SelamMQ::proxy_update_active_sns_clean(pubkey_set added, pubkey_set removed) {
...
void SelamMQ::process_zap_requests() {
    for (std::vector<zmq::message_t> frames; recv_message_parts(zap_auth, frames, zmq::recv_flags::dontwait); frames.clear()) {
        ...
        if (frames.size() < 6 || view(frames[0]) != "1.0") {
            ...
            try {
                bind_id = selamc::bt_deserialize<size_t>(view(frames[2]));
            } catch (...) {}
            ...
            if (bind_id >= bind.size()) {
                ...
                if (bind[bind_id].curve) {
                    user_id.reserve(64);
                    selamc::to_hex(pubkey.begin(), pubkey.end(), std::back_inserter(user_id));
                }
