...
std::ostream& operator<<(std::ostream& o, const ConnectionID& conn) {
    if (!conn.pk.empty())
        return o << (conn.sn() ? "SN " : "non-SN authenticated remote ") << selamc::to_hex(conn.pk);
    else
...
void SelamMQ::rebuild_pollitems() {
    pollitems.clear();
...
void SelamMQ::setup_external_socket(zmq::socket_t& socket) {
...
void SelamMQ::setup_outgoing_socket(zmq::socket_t& socket, std::string_view remote_pubkey, bool use_ephemeral_routing_id) {
...
void SelamMQ::setup_incoming_socket(zmq::socket_t& listener, bool curve, std::string_view pubkey, std::string_view privkey, size_t bind_index) {

    setup_external_socket(listener);

    listener.set(zmq::sockopt::zap_domain, selamc::bt_serialize(bind_index));
    if (curve) {
...
// Deprecated versions:
ConnectionID SelamMQ::connect_remote(std::string_view remote, ConnectSuccess on_connect,
...
void SelamMQ::disconnect(ConnectionID id, std::chrono::milliseconds linger) {
    detail::send_control(get_control_socket(), "DISCONNECT", selamc::bt_serialize<selamc::bt_dict>({
            {"conn_id", id.id},
...
SelamMQ::proxy_connect_sn(std::string_view remote, std::string_view connect_hint, bool optional, bool incoming_only, bool outgoing_only, bool use_ephemeral_routing_id, std::chrono::milliseconds keep_alive) {
    ...
    if (peer) {
        OMQ_TRACE("proxy asked to connect to ", selamc::to_hex(remote), "; reusing existing connection");
    ...
    OMQ_LOG(debug, "proxy establishing new outbound connection to ", selamc::to_hex(remote));
    std::string addr;
    ...
    if (to_self) {
        ...
        if (addr.empty()) {
            OMQ_LOG(error, "peer lookup failed for ", selamc::to_hex(remote));
            return {nullptr, ""s};
        }
    }

    OMQ_LOG(debug, selamc::to_hex(pubkey), " (me) connecting to ", addr, " to reach ", selamc::to_hex(remote));
...
std::pair<zmq::socket_t *, std::string> SelamMQ::proxy_connect_sn(selamc::bt_dict_consumer data) {
...
void SelamMQ::proxy_close_connection(int64_t id, std::chrono::milliseconds linger) {
...
void SelamMQ::proxy_expire_idle_peers() {
...
void SelamMQ::proxy_conn_cleanup() {
    ...
    for (auto it = pending_requests.begin(); it != pending_requests.end(); ) {
        ...
        if (callback.first < now) {
            OMQ_LOG(debug, "pending request ", selamc::to_hex(it->first), " expired, invoking callback with failure status and removing");
...
void SelamMQ::proxy_connect_remote(selamc::bt_dict_consumer data) {
    ...
    OMQ_LOG(debug, "Establishing remote connection to ", remote,
            remote_pubkey.empty() ? " (NULL auth)" : " via CURVE expecting pubkey " + selamc::to_hex(remote_pubkey));
...
void SelamMQ::proxy_disconnect(selamc::bt_dict_consumer data) {
...
void SelamMQ::proxy_disconnect(ConnectionID conn, std::chrono::milliseconds linger) {
