...
private:
    ...
    template <typename... T>
    friend selamc::bt_dict detail::build_send(ConnectionID to, std::string_view cmd, T&&... opts);
    friend std::ostream& operator<<(std::ostream& o, const ConnectionID& conn);
};

} // namespace selammq
namespace std {
    template <> struct hash<selammq::ConnectionID> {
        size_t operator()(const selammq::ConnectionID &c) const {
            return c.sn() ? selammq::already_hashed{}(c.pk) :
