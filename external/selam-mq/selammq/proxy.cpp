...
void SelamMQ::proxy_send(selamc::bt_dict_consumer data) {
    ...
    selamc::bt_list_consumer send = data.consume_list_consumer();
            ...
            if (!sock_route.first) {
                ...
                if (optional)
                    OMQ_LOG(debug, "Not sending: send is optional and no connection to ",
                            selamc::to_hex(conn_id.pk), " is currently established");
                else
                    OMQ_LOG(error, "Unable to send to ", selamc::to_hex(conn_id.pk), ": no valid connection address found");
                break;
            }
            ...
            if (e.num() == EHOSTUNREACH && !conn_id.route.empty() /*= incoming conn*/) {
                ...
                if (pr.first != peers.end()) {
                    if (!conn_id.sn()) {
                        ...
                        if (removed) {
                            OMQ_LOG(debug, "Retrying sending to SN ", selamc::to_hex(conn_id.pk), " using other sockets");
                            retry = true;
                        }
                    }
                }
    ...
    if (request) {
        if (sent) {
            OMQ_LOG(debug, "Added new pending request ", selamc::to_hex(request_tag));
...
void SelamMQ::proxy_reply(selamc::bt_dict_consumer data) {
    bool have_conn_id = false;
...
void SelamMQ::proxy_control_message(std::vector<zmq::message_t>& parts) {
    // We throw an uncaught exception here because we only generate control messages internally in
    // selammq code: if one of these condition fail it's a selammq bug.
    if (parts.size() < 2)
        throw std::logic_error("SelamMQ bug: Expected 2-3 message parts for a proxy control message");
    ...
    if (parts.size() == 3) {
        ...
        if (cmd == "SEND") {
            ...
            auto ptrval = selamc::bt_deserialize<uintptr_t>(data);
            return proxy_batch(reinterpret_cast<detail::Batch*>(ptrval));
        } else if (cmd == "INJECT") {
            OMQ_TRACE("proxy inject");
            return proxy_inject_task(detail::deserialize_object<injected_task>(selamc::bt_deserialize<uintptr_t>(data)));
        } else if (cmd == "SET_SNS") {
            ...
            return proxy_timer_del(selamc::bt_deserialize<int>(data));
        } else if (cmd == "BIND") {
            auto b = detail::deserialize_object<bind_data>(selamc::bt_deserialize<uintptr_t>(data));
    ...
    throw std::runtime_error("SelamMQ bug: Proxy received invalid control command: " +
            std::string{cmd} + " (" + std::to_string(parts.size()) + ")");
}

bool SelamMQ::proxy_bind(bind_data& b, size_t bind_index) {
    ...
    if (!good) {
        OMQ_LOG(warn, "SelamMQ failed to listen on ", b.address);
        return false;
    }

    OMQ_LOG(info, "SelamMQ listening on ", b.address);
...
void SelamMQ::proxy_loop_init() {
    ...
    for (size_t i = 0; i < bind.size(); i++) {
        if (!proxy_bind(bind[i], i)) {
            OMQ_LOG(fatal, "SelamMQ failed to listen on ", bind[i].address);
            throw zmq::error_t{};
        }
    }
    ...
    if (-1 == zmq_timers_add(timers.get(),
            ...
            [](int /*timer_id*/, void* self) { static_cast<SelamMQ*>(self)->proxy_conn_cleanup(); },
            this)) {
        throw zmq::error_t{};
    }
...
void SelamMQ::proxy_loop(std::promise<void> startup) {
    try {
        proxy_loop_init();
    } catch (...) {
...
// reason)
bool SelamMQ::proxy_handle_builtin(int64_t conn_id, zmq::socket_t& sock, std::vector<zmq::message_t>& parts) {
    ...
    if (cmd == "REPLY") {
        ...
        if (it != pending_requests.end()) {
            OMQ_LOG(debug, "Received REPLY for pending command ", selamc::to_hex(reply_tag), "; scheduling callback");
            ...
            OMQ_LOG(warn, "Received REPLY with unknown or already handled reply tag (", selamc::to_hex(reply_tag), "); ignoring");
        }
        return true;
    } else if (cmd == "HI") {
        if (!incoming) {
        ...
        if (parts.size() == (1 + incoming) && cmd == "UNKNOWNCOMMAND") {
            ...
            OMQ_LOG(debug, "Received plain UNKNOWNCOMMAND; remote is probably an older selammq. Ignoring.");
            return true;
        }
        ...
        if (parts.size() == (3 + incoming) && view(parts[1 + incoming]) == "REPLY") {
            ...
            auto it = pending_requests.find(reply_tag);
            if (it != pending_requests.end()) {
                OMQ_LOG(debug, "Received ", cmd, " REPLY for pending command ", selamc::to_hex(reply_tag), "; scheduling failure callback");
...
void SelamMQ::proxy_process_queue() {
