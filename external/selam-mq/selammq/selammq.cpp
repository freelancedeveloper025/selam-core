...
template <typename T>
zmq::message_t create_bt_message(T&& data) { return create_message(selamc::bt_serialize(std::forward<T>(data))); }
...
std::pair<std::string, AuthLevel> extract_metadata(zmq::message_t& msg) {
    ...
    try {
        ...
        assert(selamc::is_hex(pubkey_hex.begin(), pubkey_hex.end()));
        result.first.resize(32, 0);
        selamc::from_hex(pubkey_hex.begin(), pubkey_hex.end(), result.first.begin());
    } catch (...) {}
...
void SelamMQ::set_zmq_context_option(zmq::ctxopt option, int value) {
    context.set(option, value);
}

void SelamMQ::log_level(LogLevel level) {
    log_lvl.store(level, std::memory_order_relaxed);
}

LogLevel SelamMQ::log_level() const {
    return log_lvl.load(std::memory_order_relaxed);
}


CatHelper SelamMQ::add_category(std::string name, Access access_level, unsigned int reserved_threads, int max_queue) {
...
void SelamMQ::add_command(const std::string& category, std::string name, CommandCallback callback) {
...
void SelamMQ::add_request_command(const std::string& category, std::string name, CommandCallback callback) {
    add_command(category, name, std::move(callback));
    categories.at(category).commands.at(name).second = true;
}

void SelamMQ::add_command_alias(std::string from, std::string to) {
...
zmq::socket_t& SelamMQ::get_control_socket() {
    assert(proxy_thread.joinable());

    // Optimize by caching the last value; SelamMQ is often a singleton and in that case we're
    ...
    if (proxy_shutting_down)
        throw std::runtime_error("Unable to obtain SelamMQ control socket: proxy thread is shutting down");
        ...
        sn_lookup{std::move(lookup)}, log_lvl{level}, logger{std::move(logger)}
{
    ...
    OMQ_TRACE("Constructing SelamMQ, id=", object_id, ", this=", this);

    if (sodium_init() == -1)
        throw std::runtime_error{"libsodium initialization failed"};

    if (pubkey.empty() != privkey.empty()) {
        throw std::invalid_argument("SelamMQ construction failed: one (and only one) of pubkey/privkey is empty. Both must be specified, or both empty to generate a key.");
    } else if (pubkey.empty()) {
        if (service_node)
            throw std::invalid_argument("Cannot construct a service node mode SelamMQ without a keypair");
        OMQ_LOG(debug, "generating x25519 keypair for remote-only SelamMQ instance");
        ...
        if (verify_pubkey != pubkey)
            throw std::invalid_argument("Invalid pubkey/privkey values given to SelamMQ construction: pubkey verification failed");
    }
}
...
void SelamMQ::start() {
    ...
    OMQ_LOG(info, "Initializing SelamMQ ", bind.empty() ? "remote-only" : "listener", " with pubkey ", selamc::to_hex(pubkey));
    ...
    if (MAX_SOCKETS > 1 && MAX_SOCKETS <= zmq_socket_limit)
        ...
        OMQ_LOG(error, "Not applying SelamMQ::MAX_SOCKETS setting: ", MAX_SOCKETS, " must be in [1, ", zmq_socket_limit, "]");
    ...
    proxy_thread = std::thread{&SelamMQ::proxy_loop, this, std::move(startup_prom)};
    ...
    catch (const zmq::error_t &e) { throw std::runtime_error("Failure reading from SelamMQ::Proxy thread: "s + e.what()); }
...
void SelamMQ::listen_curve(std::string bind_addr, AllowFunc allow_connection, std::function<void(bool)> on_bind) {
    ...
    if (proxy_thread.joinable())
        detail::send_control(get_control_socket(), "BIND", selamc::bt_serialize(detail::serialize_object(std::move(d))));
    else
        bind.push_back(std::move(d));
}

void SelamMQ::listen_plain(std::string bind_addr, AllowFunc allow_connection, std::function<void(bool)> on_bind) {
    if (std::string_view{bind_addr}.substr(0, 9) == "inproc://")
...
std::pair<SelamMQ::category*, const std::pair<SelamMQ::CommandCallback, bool>*> SelamMQ::get_command(std::string& command) {
...
void SelamMQ::set_batch_threads(int threads) {
    if (proxy_thread.joinable())
...
void SelamMQ::set_reply_threads(int threads) {
    if (proxy_thread.joinable())
...
void SelamMQ::set_general_threads(int threads) {
    if (proxy_thread.joinable())
...
SelamMQ::run_info& SelamMQ::run_info::load(category* cat_, std::string command_, ConnectionID conn_, Access access_, std::string remote_,
...
SelamMQ::run_info& SelamMQ::run_info::load(category* cat_, std::string command_, std::string remote_, std::function<void()> callback) {
    reset();
    is_injected = true;
    cat = cat_;
    command = std::move(command_);
...
SelamMQ::run_info& SelamMQ::run_info::load(pending_command&& pending) {
...
SelamMQ::run_info& SelamMQ::run_info::load(batch_job&& bj, bool reply_job, int tagged_thread) {
    reset();
    is_batch_job = true;
...
SelamMQ::~SelamMQ() {
    ...
    OMQ_LOG(info, "SelamMQ shutting down proxy thread");
    detail::send_control(get_control_socket(), "QUIT");
    proxy_thread.join();
    OMQ_LOG(info, "SelamMQ proxy thread has stopped");
}
