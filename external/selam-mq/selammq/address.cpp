...
std::string decode_pubkey(std::string_view& in, bool qr) {
    std::string pubkey;
    if (in.size() >= 64 && selamc::is_hex(in.substr(0, 64))) {
        pubkey = selamc::from_hex(in.substr(0, 64));
        in.remove_prefix(64);
    } else if (in.size() >= 52 && selamc::is_base32z(in.substr(0, 52))) {
        pubkey = selamc::from_base32z(in.substr(0, 52));
        in.remove_prefix(52);
    } else if (!qr && in.size() >= 43 && selamc::is_base64(in.substr(0, 43))) {
        pubkey = selamc::from_base64(in.substr(0, 43));
        in.remove_prefix(43);
...
std::pair<std::string, std::string> parse_unix(std::string_view& addr, bool expect_pubkey) {
    ...
    if (expect_pubkey) {
        ...
        if (addr.size() > 64 && addr[addr.size() - 65] == '/' && selamc::is_hex(addr.substr(addr.size() - 64))) {
            result.first = std::string{addr.substr(0, addr.size() - 65)};
            result.second = selamc::from_hex(addr.substr(addr.size() - 64));
        } else if (addr.size() > 52 && addr[addr.size() - 53] == '/' && selamc::is_base32z(addr.substr(addr.size() - 52))) {
            result.first = std::string{addr.substr(0, addr.size() - 53)};
            result.second = selamc::from_base32z(addr.substr(addr.size() - 52));
        } else if (addr.size() > b64_len && addr[addr.size() - b64_len - 1] == '/' && selamc::is_base64(addr.substr(addr.size() - b64_len))) {
            result.first = std::string{addr.substr(0, addr.size() - b64_len - 1)};
            result.second = selamc::from_base64(addr.substr(addr.size() - b64_len));
        } else {
...
std::string address::encode_pubkey(encoding enc) const {
    std::string pk;
    if (enc == encoding::hex)
        pk = selamc::to_hex(pubkey);
    else if (enc == encoding::base32z)
        pk = selamc::to_base32z(pubkey);
    else if (enc == encoding::BASE32Z) {
        pk = selamc::to_base32z(pubkey);
        for (char& c : pk)
            if (c >= 'a' && c <= 'z')
