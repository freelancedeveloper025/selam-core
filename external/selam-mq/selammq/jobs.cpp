...
void SelamMQ::proxy_batch(detail::Batch* batch) {
...
void SelamMQ::job(std::function<void()> f, std::optional<TaggedThreadID> thread) {
    ...
    detail::send_control(get_control_socket(), "BATCH", selamc::bt_serialize(reinterpret_cast<uintptr_t>(baseptr)));
}

void SelamMQ::proxy_schedule_reply_job(std::function<void()> f) {
    auto* b = new Batch<void>;
...
void SelamMQ::proxy_run_batch_jobs(std::queue<batch_job>& jobs, const int reserved, int& active, bool reply) {
...
void SelamMQ::proxy_timer(int id, std::function<void()> job, std::chrono::milliseconds interval, bool squelch, int thread) {
    if (!timers)
        timers.reset(zmq_timers_new());

    int zmq_timer_id = zmq_timers_add(timers.get(),
            interval.count(),
            [](int timer_id, void* self) { static_cast<SelamMQ*>(self)->_queue_timer_job(timer_id); },
            this);
    if (zmq_timer_id == -1)
        throw zmq::error_t{};
...
void SelamMQ::proxy_timer(selamc::bt_list_consumer timer_data) {
...
void SelamMQ::_queue_timer_job(int timer_id) {
...
void SelamMQ::add_timer(TimerID& timer, std::function<void()> job, std::chrono::milliseconds interval, bool squelch, std::optional<TaggedThreadID> thread) {
    int th_id = thread ? thread->_id : 0;
    timer._id = next_timer_id++;
    if (proxy_thread.joinable()) {
        detail::send_control(get_control_socket(), "TIMER", selamc::bt_serialize(selamc::bt_list{{
                    timer._id,
...
TimerID SelamMQ::add_timer(std::function<void()> job, std::chrono::milliseconds interval, bool squelch, std::optional<TaggedThreadID> thread) {
    TimerID tid;
    add_timer(tid, std::move(job), interval, squelch, std::move(thread));
    return tid;
}

void SelamMQ::proxy_timer_del(int id) {
    if (!timers)
        return;
...
void SelamMQ::cancel_timer(TimerID timer_id) {
    if (proxy_thread.joinable()) {
        detail::send_control(get_control_socket(), "TIMER_DEL", selamc::bt_serialize(timer_id._id));
    } else {
        proxy_timer_del(timer_id._id);
    }
}

void SelamMQ::TimersDeleter::operator()(void* timers) { zmq_timers_destroy(&timers); }

TaggedThreadID SelamMQ::add_tagged_thread(std::string name, std::function<void()> start) {
    if (proxy_thread.joinable())
    ...
    run.worker_thread = std::thread{&SelamMQ::worker_thread, this, run.worker_id, name, std::move(start)};
