...
class Message {
public:
    SelamMQ& selammq; ///< The owning SelamMQ object
    ...
    /// Constructor
    Message(SelamMQ& omq, ConnectionID cid, Access access, std::string remote)
        : selammq{omq}, conn{std::move(cid)}, access{std::move(access)}, remote{std::move(remote)} {}
    ...
    public:
        ...
        explicit DeferredSend(Message& m) : selammq{m.selammq}, conn{m.conn}, reply_tag{m.reply_tag} {}
